const ProductDetail = () => {
  ...
  const [activeTab, setActiveTab] = useState('buy'); // 'buy' ou 'kit'
  const [locale, setLocale] = useState('pt-BR');
  const content = getContent(locale);
  const { addPerfume, selectedPerfumes, getRemainingSlots, addFullBottle, selectKit, setCartOpen } = useCart();
    ...
    const fetchProduct = () => {
      ...
      try {
        ...
        if (productsList && productsList[productIndex]) {
          ...
          setIsAddedToKit(selectedPerfumes.some(p => p.id === productIndex));
              ...
              {activeTab === 'buy' && (
                <div className="space-y-4">
                  <div className="bg-[#f7f3ec] rounded-lg p-4">
                    ...
                    <button
                      onClick={handleBuyFullBottle}
                      className="w-full py-3 bg-[#c9a96a] text-black rounded-lg font-semibold hover:bg-[#e8c98a] transition-colors flex items-center justify-center gap-2"
                    >
                      <ShoppingCart size={18} />
                      Comprar Agora
                    </button>
                    <p className="text-center text-xs text-[#1a1410]/40 mt-3">
                      Frete grátis acima de 50€ • Parcele em até 3x sem juros
                    </p>
                  </div>

                  <div className="bg-[#f7f3ec] rounded-lg p-3 border border-[#c9a96a]/20">
                    <p className="text-[#c9a96a] text-sm">
                      💰 Compre este produto e ganhe {Math.floor(product.priceValue)} Pontos!
                    </p>
                  </div>
                </div>
              )}

              {/* TAB: MONTAR KIT DECANTS */}
              {activeTab === 'kit' && (
                <div className="space-y-4">
                  {/* Aviso se não tiver kit selecionado */}
                  {!selectedKit && (
                    <div className="bg-[#1a1410] rounded-lg p-4 text-center">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <button
                          onClick={() => handleSelectKit('3ml')}
                          className="bg-[#0a0807] border border-[#c9a96a]/30 rounded-lg p-3 hover:border-[#c9a96a] transition-all"
                        >
                  ...
                  {selectedKit && (
                    <div className="bg-gradient-to-r from-[#1a1410] to-[#0a0807] rounded-lg p-4">
                      <div className="flex items-center gap-4 mb-3">
                        <img 
                          src={selectedKit === '3ml' ? '/images/decant.png' : '/images/decant1.png'} 
                          alt="Kit" 
                          className="w-16 h-16 object-contain bg-[#0a0807] rounded-full p-2 border border-[#c9a96a]/30" 
                        />
                        <div className="flex-1">
                          <p className="text-[#e8c98a] font-semibold">Kit {selectedKit}</p>
                          <p className="text-[#c9a96a] text-sm">
                            {selectedPerfumes.length}/10 perfumes selecionados
                          </p>
                          <div className="w-full bg-[#0a0807] rounded-full h-2 mt-2">
                            <div 
                              className="bg-[#c9a96a] h-2 rounded-full transition-all duration-300"
                              style={{ width: `${(selectedPerfumes.length / 10) * 100}%` }}
                            />
                          </div>
                          <p className="text-[#c9a96a]/60 text-xs mt-1">
                            Faltam {remainingSlots} perfume(s)
                          </p>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        disabled={isAddedToKit || remainingSlots === 0}
                        className={`w-full py-3 rounded-lg font-semibold flex items-center justify-center gap-2 transition-colors ${
                          isAddedToKit || remainingSlots === 0
                            ? 'bg-gray-600 cursor-not-allowed'
                            : 'bg-[#c9a96a] text-black hover:bg-[#e8c98a]'
                        }`}
                      >
                        <Package size={18} />
                        {isAddedToKit 
                          ? '✓ Já está no seu kit!' 
                          : remainingSlots === 0 
                            ? 'Kit completo! Finalize no carrinho' 
                            : `Adicionar ao Kit (${selectedPerfumes.length + 1}/10)`}
                      </button>
                    </div>
                  )}
                </div>
              ...
              {relatedProducts.map((related, idx) => (
                <Link key={idx} to={`/produto/${related.category}/${idx}`} className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow group">
                  <div className="aspect-square bg-[#f7f3ec]">
                    <img src={related.image} alt={related.name} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300" />
                  </div>
                  <div className="p-4">
                    <h3 className="font-semibold text-[#1a1410] mb-1 line-clamp-2 text-sm">{related.name}</h3>
                    <p className="text-[#c9a96a] text-sm font-medium">{related.price}</p>
                  </div>
                </Link>
              ))}
            </div>
